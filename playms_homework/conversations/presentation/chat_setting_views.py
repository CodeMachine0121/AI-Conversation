...
class ChatSettingViewSet(viewsets.ModelViewSet):
    ...
    def get_chat_settings(self):
        chat_setting = self.service.get_chat_settting(self.request.user.id)
        return Response(
    ...
    def perform_create(self, request):
        ...
        settings_data: ChatSetting = serializer.validated_data
    ...
    def update_chat_settings(self, request):
        ...
        settings_data: ChatSetting = serializer.validated_data
        user_id = self.request.user.id
        chat_setting = self.service.update_chat_setting(user_id, settings_data)
        return Response(
            ChatSettingSerializer(chat_setting).data,
            status=status.HTTP_200_OK
        )
